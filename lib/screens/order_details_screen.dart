...
class OrderDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final OrderDetailsScreen args = ModalRoute.of(context).settings.arguments;
      CollectionReference _firestore = FirebaseFirestore.instance.collection('allOrders');
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          ...
          _richText2('Oreder Number', args.model.orderNumber, Colors.red[900]),
          SizedBox(height: 10.0),
          ...
          SizedBox(height: 10.0),
          _richText2(
              'Total Price', args.model.totalPrice.toString(), Colors.blueGrey),
          SizedBox(height: 10.0),
          _richText2(
              'Pickup Station',
         ...
         Center(child: _richText2('Current Status', args.model.orderStatus, Colors.red[900])),
SizedBox(height:10.0),
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            ...
            children: [
              _statusButton(() async{
              await  _firestore.where('orderNumber',isEqualTo:args.model.orderNumber).get().then((querySnapshot) {
                  querySnapshot.docs.forEach((documentSnapshot) { 
                    documentSnapshot.reference.update({'orderStatus':'dispatched'});
                  });
                });
              }, 'DISPATCHED'),
              _statusButton(()async {
                 ...
                 await  _firestore.where('orderNumber',isEqualTo:args.model.orderNumber).get().then((querySnapshot) {
                  querySnapshot.docs.forEach((documentSnapshot) { 
                    documentSnapshot.reference.update({'orderStatus':'arrived'});
                  });
                });
              }, 'ARRIVED'),
            ],
          ...
          Row(
            ...
            children: [
              _statusButton(() async{
                 await  _firestore.where('orderNumber',isEqualTo:args.model.orderNumber).get().then((querySnapshot) {
                  querySnapshot.docs.forEach((documentSnapshot) { 
                    documentSnapshot.reference.update({'orderStatus':'delivered','deliveryDate':DateTime.now()});
                  });
                });
              }, 'DELIVERED'),
              _statusButton(() async{
                 ...
                 await  _firestore.where('orderNumber',isEqualTo:args.model.orderNumber).get().then((querySnapshot) {
                  querySnapshot.docs.forEach((documentSnapshot) { 
                    documentSnapshot.reference.update({'orderStatus':'canceled'});
                  });
                });
              }, 'CANCELED')
            ],
  ...
  ElevatedButton _statusButton(Function onPressed, String status) =>
      ElevatedButton(onPressed: onPressed, child: Text(status));
